...
def Home_Page(request):
    ...
    answers = Answer_Table.objects.select_related(
        'question_id').order_by('-created_on')
...
def Ask_Question_Page(request):
    if (request.method == 'POST'):
        ...
        if (len(message) > 0):
            ...
            question = message
            data = Question_Table(
                question_text=question, asked_by_user_id=user_id, asked_by_user_name=user_name)
            data.save()
...
def Add_Like_Page(request, answer_id, answered_by_user_id):
    ...
    if existing_like is None:
        like = Like_Table(answer_id=answer,
